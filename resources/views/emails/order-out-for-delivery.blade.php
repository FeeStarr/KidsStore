...
<head>
    ...
    <title>Order On Its Way - {{ $order->reference }}</title>
</head>
...
<table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;">
    ...
    <tr>
        <td style="background-color:#2563eb;padding:24px 30px;text-align:center;">
            <h1 style="margin:0;color:#ffffff;font-size:20px;font-weight:600;">Your Order Is On Its Way!</h1>
    ...
    <tr>
        <td style="padding:28px 30px 0;">
            ...
            @if($order->courier_name)
                <p style="margin:10px 0 0;font-size:14px;color:#4b5563;line-height:1.6;">
                    Delivered by: <strong>{{ $order->courier_name }}</strong>
                    @if($order->tracking_number)
                        &mdash; Tracking: <strong>{{ $order->tracking_number }}</strong>
                    @endif
                </p>
            @endif
        </td>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f9fafb;border-radius:6px;border:1px solid #e5e7eb;">
                    ...
                    <td style="padding:16px 20px;border-bottom:1px solid #e5e7eb;">
                        <table width="100%" cellpadding="0" cellspacing="0">
                            <tr>
                                ...
                                <td style="font-size:13px;color:#1f2937;text-align:right;font-weight:600;">{{ $order->order_date->format('M d, Y g:i A') }}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
                ...
                <tr>
                    <td style="padding:12px 20px;border-bottom:1px solid #e5e7eb;">
                        <table width="100%" cellpadding="0" cellspacing="0">
                            <tr>
                                <td style="font-size:13px;color:#6b7280;">Delivery Address</td>
                                <td style="font-size:13px;color:#1f2937;text-align:right;font-weight:600;">{{ $order->delivery_address }}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
    ...
    <tr>
        <td style="padding:24px 30px 0;">
            <h2 style="margin:0 0 14px;font-size:15px;color:#1f2937;border-bottom:1px solid #e5e7eb;padding-bottom:10px;">Items In This Order</h2>
            <table width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td style="padding:10px 0 10px 12px;border-bottom:1px solid #f3f4f6;vertical-align:top;">
                        ...
                        @if($item->selected_size)
                            <div style="font-size:12px;color:#6b7280;margin:2px 0 0;">Size: {{ $item->selected_size }}</div>
                        @endif
                    ...
                    <td style="padding:10px 0;border-bottom:1px solid #f3f4f6;vertical-align:top;text-align:right;white-space:nowrap;">
                        <div style="font-size:13px;color:#4b5563;">{{ $item->quantity }} x &#8358;{{ number_format($item->unit_price, 2) }}</div>
    ...
    <tr>
        <td style="padding:24px 30px 0;text-align:center;">
            @if($order->tracking_url)
                <a href="{{ $order->tracking_url }}" style="display:inline-block;background-color:#2563eb;color:#ffffff;text-decoration:none;padding:12px 28px;border-radius:6px;font-size:14px;font-weight:600;">Track Your Package</a>
            @endif
            <a href="{{ url('/account/orders/' . $order->id) }}" style="display:inline-block;background-color:{{ $order->tracking_url ? '#ffffff' : '#2563eb' }};color:#2563eb;text-decoration:none;padding:12px 28px;border-radius:6px;font-size:14px;font-weight:600;border:1px solid #2563eb;margin-left:{{ $order->tracking_url ? '8px' : '0' }};">View Your Order</a>
        </td>
    </tr>
    ...
    <tr>
        <td style="padding:24px 30px 0;">
            <div style="background-color:#f9fafb;border:1px solid #e5e7eb;border-radius:6px;padding:16px 20px;">
                <p style="margin:0 0 8px;font-size:13px;color:#1f2937;font-weight:600;">Need Help? Contact Us</p>
                @if($contactPhone)
