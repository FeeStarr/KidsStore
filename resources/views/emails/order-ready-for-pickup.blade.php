...
<head>
    ...
    <title>Order Ready for Pickup — {{ $order->reference }}</title>
</head>
...
<table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;">
    ...
    <tr>
        <td style="background-color:#2563eb;padding:24px 30px;text-align:center;">
            <h1 style="margin:0;color:#ffffff;font-size:20px;font-weight:600;">Order Ready for Pickup</h1>
    ...
    <tr>
        <td style="padding:28px 30px 0;">
            ...
            <p style="margin:10px 0 0;font-size:14px;color:#4b5563;line-height:1.6;">
                Great news! Your order is now ready for pickup at the station below.
            </p>
        </td>
    </tr>
    ...
    <tr>
        <td style="padding:20px 30px 0;">
            <div style="background-color:#eff6ff;border:1px solid #bfdbfe;border-radius:6px;padding:14px 18px;">
                <p style="margin:0 0 6px;font-size:14px;color:#1e40af;font-weight:600;">{{ $order->pickupStation->name }}</p>
                @if($order->pickupStation->address)
                    <p style="margin:0;font-size:13px;color:#1e40af;">{{ $order->pickupStation->address }}</p>
                @endif
                @if($order->pickupStation->instructions)
                    <p style="margin:8px 0 0;font-size:12px;color:#6b7280;font-style:italic;">{{ $order->pickupStation->instructions }}</p>
                @endif
            </div>
        </td>
    </tr>
    ...
    <tr>
        <td style="padding:24px 30px 0;">
            <h2 style="margin:0 0 14px;font-size:15px;color:#1f2937;border-bottom:1px solid #e5e7eb;padding-bottom:10px;">Items Ready for Pickup</h2>
            <table width="100%" cellpadding="0" cellspacing="0">
    ...
    <tr>
        <td style="padding:0 30px 24px;">
            <p style="margin:0;font-size:13px;color:#6b7280;line-height:1.6;text-align:center;">
                Please bring a valid ID and your order reference when collecting your items. Items not collected within 7 days may be returned.
            </p>
        </td>
    </tr>
